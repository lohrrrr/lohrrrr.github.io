window.Kernel = {
  ...
  fs: {
    ...
    async init() {
      ...
      this.ensureDesktopDir();
    },

    ensureDesktopDir() {
      var gfs = this.getGuestStorage();
      if (!gfs.dirs.includes('Desktop')) {
        gfs.dirs.push('Desktop');
    ...
    fallbackManifest() {
      return {
        directories: [
          "/", "/bin", "/etc", "/sys", "/home", "/home/lohrrrr", "/home/guest", "/home/guest/Desktop", "/filesystem/Apps"
        ],
        files: {
    ...
    getGuestStorage() {
      ...
      if (!raw) {
        var initial = {
          dirs: ["Desktop"],
          files: {
            "Desktop/readme.txt": "Welcome to the desktop!\nFiles and folders can be created via right-click.\nAll files are stored in ~/Desktop and persist in localStorage.\nMASTERPOS itself are trying to look like a windows but don't be tricked\nbecause it's trying to implement UNIXv7 and Darwin functionality."
          }
        };
  ...
  desktop: {
    ...
    getStoredPositions() {
      var raw = localStorage.getItem('masterpos_desktop_positions');
      try {
    ...
    savePosition(key, col, row) {
      var pos = this.getStoredPositions();
      pos[key] = { col: col, row: row };
      localStorage.setItem('masterpos_desktop_positions', JSON.stringify(pos));
    },

    snapToGrid(x, y) {
      var col = Math.max(0, Math.round((x - this.baseX) / this.gridW));
      var row = Math.max(0, Math.round((y - this.baseY) / this.gridH));
      return {
        col: col,
        row: row,
        left: this.baseX + col * this.gridW,
        top: this.baseY + row * this.gridH
      };
    },
      ...
      el.addEventListener('mousedown', function(e) {
        ...
        function onMouseUp() {
          ...
          if (isDragging) {
            ...
            var snapped = self.snapToGrid(el.offsetLeft, el.offsetTop);
            el.style.left = snapped.left + 'px';
            el.style.top = snapped.top + 'px';
            self.savePosition(key, snapped.col, snapped.row);
          }
        }
    ...
    startRename(el, oldName, isDir) {
      var labelEl = el.querySelector('.icon-label');
      if (!labelEl) return;
      ...
      async function commit() {
        ...
        if (newName && newName !== oldName) {
          var oldPath = '/home/guest/Desktop/' + oldName;
          var newPath = '/home/guest/Desktop/' + newName;
          try {
            if (isDir) {
              var gfs = Kernel.fs.getGuestStorage();
              var oldRel = 'Desktop/' + oldName;
              var newRel = 'Desktop/' + newName;
              var idx = gfs.dirs.indexOf(oldRel);
              if (idx !== -1) gfs.dirs[idx] = newRel;
              for (var f in gfs.files) {
                if (f.startsWith(oldRel + '/')) {
                  gfs.files[newRel + f.substring(oldRel.length)] = gfs.files[f];
                  delete gfs.files[f];
                }
              }
              Kernel.fs.saveGuestStorage(gfs);
            } else {
              var content = await Kernel.fs.read(oldPath);
              await Kernel.fs.write(newPath, content);
              await Kernel.fs.remove(oldPath);
            }

            var pos = Kernel.desktop.getStoredPositions();
            if (pos['file:' + oldName]) {
              pos['file:' + newName] = pos['file:' + oldName];
              delete pos['file:' + oldName];
              localStorage.setItem('masterpos_desktop_positions', JSON.stringify(pos));
            }
          } catch (err) {
            alert(err.message);
    ...
    async render(focusRenameItem) {
      var grid = document.getElementById('icon-grid');
      grid.innerHTML = '';

      var positions = this.getStoredPositions();
      var usedSlots = {};

      function getFreeSlot(prefCol, prefRow) {
        var c = prefCol || 0;
        var r = prefRow || 0;
        while (usedSlots[c + ':' + r]) {
          r++;
      ...
      var appKeys = Object.keys(Kernel.apps);
      for (var a = 0; a < appKeys.length; a++) {
        var app = Kernel.apps[appKeys[a]];
        var appKey = 'app:' + app.id;
        var slot;

        if (positions[appKey]) {
          slot = positions[appKey];
          usedSlots[slot.col + ':' + slot.row] = true;
        } else {
          slot = getFreeSlot(0, a);
        }

        this.renderAppIcon(grid, app, appKey, slot);
      }

      try {
        var desktopFiles = await Kernel.fs.list('/home/guest/Desktop');
        for (var f = 0; f < desktopFiles.length; f++) {
          var item = desktopFiles[f];
          var isDir = item.endsWith('/');
          var name = isDir ? item.slice(0, -1) : item;
          var fileKey = 'file:' + name;

          var fSlot;
          if (positions[fileKey]) {
            fSlot = positions[fileKey];
            usedSlots[fSlot.col + ':' + fSlot.row] = true;
          } else {
            fSlot = getFreeSlot(1, f);
          }

          var el = this.renderFileIcon(grid, name, isDir, fileKey, fSlot);
          if (focusRenameItem && focusRenameItem === name) {
            this.startRename(el, name, isDir);
          }
        }
      } catch (e) {}
    },

    renderAppIcon(grid, app, key, slot) {
      var el = document.createElement('div');
      el.className = 'desktop-icon';
      el.setAttribute('data-target', 'win-' + app.id);
      el.style.left = (this.baseX + slot.col * this.gridW) + 'px';
      el.style.top = (this.baseY + slot.row * this.gridH) + 'px';

      el.innerHTML =
        '<div class="icon-pic">' + app.icon + '</div>' +
        '<div class="icon-label">' + app.title.split(' ')[0] + '</div>';
    ...
    renderFileIcon(grid, name, isDir, key, slot) {
      var self = this;
      var el = document.createElement('div');
      el.className = 'desktop-icon';
      el.style.left = (this.baseX + slot.col * this.gridW) + 'px';
      ...
      var iconSymbol = isDir ? '📁' : '📄';
      if (name.endsWith('.txt')) iconSymbol = '📝';
      if (name.endsWith('.js')) iconSymbol = '📜';

      el.innerHTML =
        '<div class="icon-pic">' + iconSymbol + '</div>' +
      ...
      var targetPath = '/home/guest/Desktop/' + name;
      ...
      el.addEventListener('contextmenu', function(e) {
        e.preventDefault();
        e.stopPropagation();
        self.activeContextItem = { el: el, name: name, isDir: isDir, path: targetPath };

        var iconMenu = document.getElementById('icon-context-menu');
        var deskMenu = document.getElementById('desktop-context-menu');
        deskMenu.style.display = 'none';

        var x = e.clientX;
        var y = e.clientY;
        if (x + 160 > window.innerWidth) x = window.innerWidth - 165;
        if (y + 100 > window.innerHeight - 30) y = window.innerHeight - 120;
    ...
    initContextMenu() {
      ...
      desktop.addEventListener('contextmenu', function(e) {
        if (e.target.closest('.window') || e.target.closest('#taskbar') || e.target.closest('#start-menu') || e.target.closest('.desktop-icon')) {
          return;
        }
        e.preventDefault();
        ...
        var y = e.clientY;
        if (x + 160 > window.innerWidth) x = window.innerWidth - 165;
        if (y + 140 > window.innerHeight - 30) y = window.innerHeight - 170;
      ...
      document.addEventListener('click', function(e) {
        if (!deskMenu.contains(e.target)) deskMenu.style.display = 'none';
        if (!iconMenu.contains(e.target)) iconMenu.style.display = 'none';
      });
      ...
      document.getElementById('ctx-open-exp').addEventListener('click', function() {
        deskMenu.style.display = 'none';
        Kernel.wm.open('win-explorer');
      });

      document.getElementById('ctx-open-np').addEventListener('click', function() {
        deskMenu.style.display = 'none';
        Kernel.wm.open('win-notepad');
      });

      document.getElementById('ctx-new-folder').addEventListener('click', async function() {
        deskMenu.style.display = 'none';
        var baseName = 'New Folder';
        var counter = 1;
        var existing = await Kernel.fs.list('/home/guest/Desktop');

        while (existing.includes(baseName + '/')) {
          counter++;
          baseName = 'New Folder (' + counter + ')';
        }

        try {
          await Kernel.fs.mkdir('/home/guest/Desktop/' + baseName);
          await self.render(baseName);
        } catch (err) {
          alert(err.message);
        }
      });

      document.getElementById('ctx-new-file').addEventListener('click', async function() {
        deskMenu.style.display = 'none';
        var baseName = 'New Document.txt';
        var counter = 1;
        var existing = await Kernel.fs.list('/home/guest/Desktop');

        while (existing.includes(baseName)) {
          counter++;
          baseName = 'New Document (' + counter + ').txt';
        }

        try {
          await Kernel.fs.write('/home/guest/Desktop/' + baseName, '');
          await self.render(baseName);
        } catch (err) {
          alert(err.message);
        }
      ...
      document.getElementById('icon-ctx-open').addEventListener('click', function() {
        iconMenu.style.display = 'none';
        if (!self.activeContextItem) return;
        if (self.activeContextItem.isDir) {
          Kernel.wm.open('win-explorer');
        } else {
          Kernel.openFileInNotepad(self.activeContextItem.path);
        }
      });

      document.getElementById('icon-ctx-rename').addEventListener('click', function() {
        iconMenu.style.display = 'none';
        if (!self.activeContextItem) return;
        self.startRename(self.activeContextItem.el, self.activeContextItem.name, self.activeContextItem.isDir);
      });

      document.getElementById('icon-ctx-delete').addEventListener('click', async function() {
        iconMenu.style.display = 'none';
        if (!self.activeContextItem) return;
        try {
          await Kernel.fs.remove(self.activeContextItem.path);
          self.render();
        } catch (err) {
          alert(err.message);
        }
    ...
    async loadApp(appFolder) {
      ...
      try {
        ...
        var meta = {
          ...
          resizable: metaEl.getAttribute('data-resizable') || 'false',
          desc: metaEl.getAttribute('data-desc') || ''
        };

        Kernel.apps[appId] = meta;
        ...
        Kernel.wm.createWindow(meta, contentHtml);
        this.createStartItem(meta, 'win-' + meta.id);
  ...
  async openFileInNotepad(path) {
    Kernel.wm.open('win-notepad');
    if (window.Notepad && typeof window.Notepad.loadFile === 'function') {
      await window.Notepad.loadFile(path);
    } else {
  ...
  async performLogin() {
    ...
    var logonScreen = document.getElementById('logon-screen');
    var desktop = document.getElementById('desktop');
  ...
  async boot() {
    ...
    await this.desktop.render();
    this.desktop.initContextMenu();
    this.initClock();
    this.initStartMenu();
  },
  ...
  initStartMenu() {
    ...
    document.getElementById('start-shutdown-btn').addEventListener('click', function() {
      if (confirm('Shut down MASTERPOS Terminal?')) {
        document.body.innerHTML = '<div style="background:#000;color:#dfc272;height:100vh;display:flex;align-items:center;justify-content:center;font-family:monospace;font-size:18px;text-align:center;padding:20px;">IT IS NOW SAFE TO TURN OFF YOUR COMPUTER.</div>';
      }
    });
  }
};
...
window.addEventListener('DOMContentLoaded', function() {
  var loginBtn = document.getElementById('logon-btn');
  var passInput = document.getElementById('logon-pass');
